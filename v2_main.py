...
class NeeqNewsScan():
    ...
    def req(self):
        ...
        try:
            headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 6.3; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0'}
            req = requests.post(url, data, timeout=8, headers=headers)
            if req.status_code == 200:
                return json.loads(req.text[5:-1])[0]['data']['content'][0]
            else:
                return False
        except Exception as e:
            self.logger.error('连接请求失败 : %s' % e)
            return False
    ...
    def run(self):
        ...
        while block_neeq:
            ...
            if content == False:
                time.sleep(float(self.timer))
                continue
            data = content['subTitle']+'---'+content['metaDescription']
            if self.chk_content(data):
                self.logger.info(data)
                self.save_content(data)
                for mail_to in self.mail_to_list:
                    if self.mail.send_mail(self.from_user, mail_to, content['subTitle'], content['metaDescription']):
                        self.logger.info('邮件发送成功-'+mail_to)
                    else:
                        self.logger.error('邮件发送失败-'+mail_to)
            if block_neeq == False:
                self.logger.info('扫描结束')
                quit()
            time.sleep(float(self.timer))
...
"""
    ...
    def req(self, data):
        ...
        try:
            ...
            req = requests.post(url, data, timeout=8, headers=headers)
            if req.status_code == 200:
                return json.loads(req.text[5:-1])[0]['listInfo']
            else:
                return False
        except Exception as e:
            self.logger.error('连接请求失败 : %s' % e)
    ...
    #解析页码
    def parse_page(self):
        content = self.req(self.data)
        if content != False:
            return content['totalPages']
        else:
            self.logger.error('页码解析错误')
            return False
    ...
    def loading(self):
        ...
        try:
            ...
            self.logger.info("配置加载失败 : %s" % e)
            return False
        #主要是抓取连接参数的获取这里对抓取量进行了限制，就最早的公告来说应该是06年左右，如果要全部抓取，进行end_time 设定即可
        end_time = time.strftime("%Y-%m-%d")
        ago = (datetime.datetime.now() - datetime.timedelta(days = 30))
        start_time = ago.strftime("%Y-%m-%d")
        self.data ={
            'companyCd' : '',
            'disclosureType' : 5,
            'endTime': end_time,
        ...
        #进行第一次入库判断
        if not os.path.exists('chk.txt'):
            self.flag_chk = 0
        else:
            self.flag_chk = 1
            with open('chk.txt', 'r') as f:
                self.flag_line = f.readline()


    #计数统计器,0代表开始，1代表结束,2代表中断,中断貌似不需要了
    def account(self, flag=0):
        if flag == 0:
            #时间统计
            ...
            use_time = int(self.end_time - self.start_time)
            msg = """
-------------------------

采集公告个数 : %s
本次采集耗时 : %s
        """%(self.gg_count, use_time)
            self.logger.info(msg)
            self.logger.removeHandler(self.fh)
    ...
    def run(self):
        while True:
            ...
            with codecs.open('conf.ini', encoding="utf-8-sig" ) as f:
                conf.readfp(f)
            timer = float(conf.get("info_cron", "timer"))
            time.sleep(timer)
        ...
        for page in range(0, pages):
            ...
            for row in rows:
                if block_info == False:
                    ...
                    quit()
                code = row['companyCd']
                title = row['disclosureTitle']
                date = row['publishDate']
                link_add = 'www.neeq.com.cn'+row['destFilePath']
    ...
    def _cron(self, e):
        ...
        while block_xiniu:
            ...
            if cron == tstr:
                ...
                continue
            time.sleep(1)
            logger.warning('等待中 : %s' % time.strftime('%H:%M:%S'))
